...
A language object has two member fields: a name, which is the name of the
language, and a trigram frequency, which is a map which holds the counts for
each possible trigram in a 27 character alphabet.
*/

class language {
public:
  /*
  Constructor with separate arguments for name and text, which is used to
  compute the trigram frequency If `text` contains invalid characters, throws a
  std::runtime_error
  */
...
private:
  /*
  Helper that takes in a pointer to a vector, representing a pointer to a
  trigram. Updates the local frequency based on currently read in buffer.
  */
